...
-- Üretim Zamanı: 14 Mar 2024, 12:53:22
...
(1, '11', '111', 1, 0, '2024-03-14 12:23:54');
...
DELIMITER $$
CREATE TRIGGER `trg_brands_delete` AFTER DELETE ON `brands` FOR EACH ROW insert into `Backend_Project`.`brands_logs`(
	`trigger`,
	brand_id,
	before_img_url,
	before_title,
...
DELIMITER $$
CREATE TRIGGER `trg_brands_insert` AFTER INSERT ON `brands` FOR EACH ROW insert into `Backend_Project`.`brands_logs`(
	`trigger`,
	brand_id,
	after_img_url,
	after_title,
...
DELIMITER $$
CREATE TRIGGER `trg_brands_update` AFTER UPDATE ON `brands` FOR EACH ROW insert into `Backend_Project`.`brands_logs`(
	brand_id,
	`trigger`,
	before_img_url,
	before_title,
...
DELIMITER $$
CREATE TRIGGER `trg_products_delete` AFTER DELETE ON `products` FOR EACH ROW insert into `Backend_Project`.`products_logs`(
	product_id,
	`trigger`,
	before_img_url,
	before_title,
...
DELIMITER $$
CREATE TRIGGER `trg_products_insert` AFTER INSERT ON `products` FOR EACH ROW insert into `Backend_Project`.`products_logs`(
	product_id,
	`trigger`,
	after_img_url,
	after_title,
...
DELIMITER $$
CREATE TRIGGER `trg_products_update` AFTER UPDATE ON `products` FOR EACH ROW insert into `Backend_Project`.`products_logs`(
	product_id,
	`trigger`,
	before_img_url,
	before_title,
...
(1, 'Kıyafet', 1, '2024-03-14 12:47:11'),
...
DELIMITER $$
CREATE TRIGGER `trg_product_images_delete` AFTER DELETE ON `product_images` FOR EACH ROW insert into `Backend_Project`.`product_images_logs`(
	product_image_id,
	`trigger`,
	before_product_id,
	before_rank,
...
DELIMITER $$
CREATE TRIGGER `trg_product_images_insert` AFTER INSERT ON `product_images` FOR EACH ROW insert into `Backend_Project`.`product_images_logs`(
	product_image_id,
	`trigger`,
	after_product_id,
	after_rank,
...
DELIMITER $$
CREATE TRIGGER `trg_product_images_update` AFTER UPDATE ON `product_images` FOR EACH ROW insert into `Backend_Project`.`product_images_logs`(
	product_image_id,
	`trigger`,
	before_product_id,
	before_rank,
...
DELIMITER $$
CREATE TRIGGER `trg_references_delete` AFTER DELETE ON `references` FOR EACH ROW insert into `Backend_Project`.`references_logs`(
	reference_id,
	`trigger`,
	before_img_url,
	before_title,
...
DELIMITER $$
CREATE TRIGGER `trg_references_insert` AFTER INSERT ON `references` FOR EACH ROW insert into `Backend_Project`.`references_logs`(
	reference_id,
	`trigger`,
	after_img_url,
	after_title,
...
DELIMITER $$
CREATE TRIGGER `trg_references_update` AFTER UPDATE ON `references` FOR EACH ROW insert into `Backend_Project`.`references_logs`(
	reference_id,
	`trigger`,
	before_img_url,
	before_title,
...
DELIMITER $$
CREATE TRIGGER `trg_services_delete` AFTER DELETE ON `services` FOR EACH ROW insert into `Backend_Project`.`services_logs`(
	service_id,
	`trigger`,
	before_img_url,
	before_title,
...
DELIMITER $$
CREATE TRIGGER `trg_services_insert` AFTER INSERT ON `services` FOR EACH ROW insert into `Backend_Project`.`services_logs`(
	service_id,
	`trigger`,
	after_img_url,
	after_title,
...
DELIMITER $$
CREATE TRIGGER `trg_services_update` AFTER UPDATE ON `services` FOR EACH ROW insert into `Backend_Project`.`services_logs`(
	service_id,
	`trigger`,
	before_img_url,
	before_title,
...
DELIMITER $$
CREATE TRIGGER `trg_settings_delete` AFTER DELETE ON `settings` FOR EACH ROW insert into `Backend_Project`.`settings_logs`(
	setting_id,
	`trigger`,
	before_company_name,
	before_adress,
...
DELIMITER $$
CREATE TRIGGER `trg_settings_insert` AFTER INSERT ON `settings` FOR EACH ROW insert into `Backend_Project`.`settings_logs`(
	setting_id,
	`trigger`,
	after_company_name,
	after_adress,
...
DELIMITER $$
CREATE TRIGGER `trg_settings_update` AFTER UPDATE ON `settings` FOR EACH ROW insert into `Backend_Project`.`settings_logs`(
	setting_id,
	`trigger`,
	before_company_name,
	before_adress,
...
DELIMITER $$
CREATE TRIGGER `trg_testiminals_delete` AFTER DELETE ON `testimanials` FOR EACH ROW insert into `Backend_Project`.`testimanials_logs`(
	testimanial_id,
	`trigger`,
	before_title,
	before_description,
...
DELIMITER $$
CREATE TRIGGER `trg_testiminals_insert` AFTER INSERT ON `testimanials` FOR EACH ROW insert into `Backend_Project`.`testimanials_logs`(
	testimanial_id,
	`trigger`,
	after_title,
	after_description,
...
DELIMITER $$
CREATE TRIGGER `trg_testiminals_update` AFTER UPDATE ON `testimanials` FOR EACH ROW insert into `Backend_Project`.`testimanials_logs`(
	testimanial_id,
	`trigger`,
	before_title,
	before_description,
...
DELIMITER $$
CREATE TRIGGER `trg_users_delete` AFTER DELETE ON `users` FOR EACH ROW insert into `Backend_Project`.`users_logs`(
	user_id,
	`trigger`,
	before_img_url,
	before_email,
...
DELIMITER $$
CREATE TRIGGER `trg_users_insert` AFTER INSERT ON `users` FOR EACH ROW insert into `Backend_Project`.`users_logs`(
	`trigger`,
	user_id,
	after_img_url,
	after_email,
...
DELIMITER $$
CREATE TRIGGER `trg_users_update` AFTER UPDATE ON `users` FOR EACH ROW insert into `Backend_Project`.`users_logs`(
	user_id,
	`trigger`,
	before_img_url,
	before_email,
